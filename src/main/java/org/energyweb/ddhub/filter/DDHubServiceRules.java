...
public class DDHubServiceRules implements ContainerRequestFilter {
	...
	public void filter(ContainerRequestContext requestContext) throws IOException {
		if(rulesConfig.ddhubNamespace().isEmpty()) return;
    	...
    	try {
			...
			if(jsonArray.toJSONString().indexOf(rulesConfig.ddhubNamespace().get()) < 0) {
			...
			jsonArray.forEach(item -> {
			    ...
			    if(namespace.indexOf(rulesConfig.ddhubNamespace().get()) < 0) {
			    	...
			    	hubService.ifPresent(service ->{
			    		if(service.rules().stream().filter(str->namespace.matches(str.replace("*", "(\\w*|\\w*[-]\\w*|\\w*[-]\\w*[-]\\w*)"))).findFirst().isEmpty()) {
			    			requestContext.abortWith(Response.status(Response.Status.UNAUTHORIZED).entity(new ErrorResponse("10", "Unauthorize access")).build());
			    		}
			    	});
			    }
